...
pub(crate) unsafe fn get_win_os_error(msg: &str) -> String {
...
pub(crate) unsafe extern "system" fn window_proc(
    ...
    if msg == U_TASKBAR_RESTART {
        let icon: HICON = unsafe {
            ...
            if handle == 0 {
                println!("Error setting icon from resource 3");
                PostQuitMessage(0);
            }
            handle as HICON
        };
...
pub(crate) unsafe fn init_window() -> Result<WindowInfo> {
    ...
    if hmodule == 0 {
        bail!(get_win_os_error("Error getting module handle"));
    }
    ...
    if hwnd == 0 {
        bail!(get_win_os_error("Failed at creating window"));
    }
    ...
    let icon: HICON = unsafe {
        ...
        if handle == 0 {
            bail!("Failed at loading icon for tray");
        }
        handle as HICON
    };
    ...
    if hmenu == 0 {
        bail!("Failed at creating popup menu");
    }

    if SetMenuInfo(hmenu, &info) == 0 {
        bail!("Failed at setting up menu");
    }
